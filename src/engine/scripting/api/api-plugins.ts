...
import { ResourcePath, GameResource } from "../../core/resource";
import { PluginManager } from "../../plugin/plugin-manager";
import { AVGPlugin } from "../../plugin/avg-plugin";

export namespace plugins {
  export async function load(file: string) {
    const scripts = await AVGNativeFS.readFileSync(AVGNativePath.join(GameResource.getPath(ResourcePath.Plugins), file));
